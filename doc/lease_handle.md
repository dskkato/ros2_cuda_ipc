...
struct ShmHeader {
  ...
  uint32_t free_head;       // フリーリスト先頭 (UINT32_MAX = 空)
};

struct SlotMeta {
  uint32_t generation;  // as_atomic() で std::atomic<uint32_t> として扱う
  uint32_t refcnt;      // 同上
  uint32_t next_free;   // フリーリストで使用
};
...
崩さずにアトミック操作だけを付与している。`free_head` と `next_free` でロックフリーなリスト（Treiber
stack 方式の単方向スタック）を構成し、Publisher は空き slot を定数時間で `pop` / `push` できる。
...
2. 発行準備
`LeaseHandle::choose_empty_slot()` でフリーリストから slot を `pop`。選択した slot の
generation++（publish 直前の世代進行）。
...
5. 再利用チェック
subscriber が解放すると自動でフリーリストへ戻るため、Publisher 側は枯渇時のみ
`choose_empty_slot()` の戻り値が `std::nullopt` になることを確認すればよい。
...
0 になった瞬間にフリーリストへ `push` され、Publisher から再度取得可能となる。
...
public:
  ...
  // Publisher が失敗した slot をフリーリストへ戻す
...
* `generation`（Publisher 側の更新）：`store(new_value, std::memory_order_release)` → publish  
  * publish 後はSubscriber側が新世代を確実に観測できる関係を担保
...
ロックフリーな単方向スタックを実現する古典的な設計で、Robert Treiber が 1986 年に IBM で発表したメモに由来します。主要なポイントは以下のとおりです。

* **構造**: スタックのトップを指す原子的なポインタ（ここでは `ShmHeader::free_head`）と、各ノードが次ノードへのリンク（`SlotMeta::next_free`）を持つ単方向リスト。
* **push 操作**: 現在のヘッドを読み、新ノードの `next` に設定した上で `compare_exchange`（CAS）でヘッドを新ノードに更新。CAS が失敗したら最新のヘッドを読み直してリトライ。
* **pop 操作**: ヘッドを読み、次ノードを保持してから `compare_exchange` でヘッドを次ノードへ更新。こちらも CAS が失敗したら再試行。
* **特長**: グローバルロックが不要で、他スレッドの停止や遅延に影響されず、空きスロットの取得・返却が定数時間で行える。ABA 問題対策が必要なケースもあるが、本実装では世代番号で slot 再利用を判定する他、push/pop は単一 Publisher と複数 Subscriber の組み合わせに限定されているためシンプルに運用できる。

今回の `LeaseHandle` では、この Treiber stack パターンを共有メモリ上に展開し、slot の空きリスト管理に利用している。
...
sequenceDiagram
  ...
  Pub->>LHp: choose_empty_slot()  // refcnt==0 を選ぶ
  LHp-->>Pub: slot_id
  Pub->>LHp: bump_generation(slot_id)  // generation++
