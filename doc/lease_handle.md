...
2. 発行準備
空き slot を選ぶ（refcnt==0 を確認）。選択した slot の generation++（publish 直前の世代進行）。
...
5. 再利用チェック
次回同じ slot を使う前に 必ず refcnt==0 を再確認。0 でなければその slot はスキップ／隔離（運用ポリシーに従う）。
...
* View の RAII により、スコープ終了で自動的に`refcnt--`。
...
classDiagram
  ...
  class BufferView {
    ...
    +uint32      slot_id
    +uint32      generation
    +shared_ptr<void> lease  // LeaseHandleを保持
