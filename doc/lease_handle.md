...
### リソース対応関係（1 Publisher / 1 Subscriber）

```
Publisher process                             ROS 2 transport (BufferCore)                    Subscriber process
+-----------------------------------------+   +--------------------------------------------+   +--------------------------------------------+
| GPU VRAM slot pool (slot_count = N)     |   | BufferCore {                                |   | LeaseHandle::acquire(shm, slot, gen)       |
|  Slot 0: [dev_ptr0][event0][IPC handles]|   |   shm_name = "/topic_shm"                   |   |   ├─ cudaIpcOpenMemHandle -> dev_ptr0      |
|  Slot 1: [dev_ptr1][event1][IPC handles]|   |   slot_id = 0                                |   |   ├─ cudaIpcOpenEventHandle -> event0      |
|  ...                                    |   |   generation = g                            |   |   └─ refcnt++ on SlotMeta[0]               |
+-----------------------------------------+   |   mem_handle  (GPU memory handle for Slot 0)|   +--------------------------------------------+
             |                                 |   event_handle (CUDA event handle for Slot 0)|
             | cudaIpcGet*Handle               |   byte_size, device_id, ...                  |   Shared memory mapping
             v                                 +--------------------------------------------+   +--------------------------------------------+
Shared memory (/topic_shm)                                                                   | SlotMeta[0]: {generation=g, refcnt, pending}|
+-----------------------------------------+                                                     | SlotMeta[1]: {...}                          |
| ShmHeader: capacity=N, consumers=1      |<-------------------------------------------------->| ...                                        |
| SlotMeta[0]: {generation, refcnt, ...}  |   (BufferCore が shm_name/slot_id を伝達)          +--------------------------------------------+
| SlotMeta[1]: {...}                      |
+-----------------------------------------+
```

* BufferCore メッセージは GPU メモリと CUDA イベントの IPC ハンドルを **両方** 保持し、同時に共有メモリ上のスロットを特定するための `shm_name` と `slot_id`、`generation` を伝達する。
* Subscriber は BufferCore を受信すると、共有メモリ上の `SlotMeta` に対して参照カウントを更新しつつ、同じメッセージに含まれる IPC ハンドルを使って GPU リソースへアクセスする。
* 共有メモリ自体にはハンドルは格納されず、Publisher/Subscriber が BufferCore を介して同じスロットを参照するためのインデックス情報のみを保持する。
