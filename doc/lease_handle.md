...
CUDA IPC ハンドルや VMM import の結果は BufferView/MemoryBackend 側のプロセス内キャッシュで共有し、
初回だけ `cudaIpcOpen*Handle` などを呼んで dev_ptr / event を取得する。以降は同じハンドルを
...
* 各メッセージ（`GpuImage`, `GpuPointCloud2`）内の`BufferCore`が以下を提供：
  ...
  * `generation`（再利用時に増加）
* Subscriber はこの3つをキーに Lease を獲得（=`refcnt++`）し、破棄時に `refcnt--` する。
...
受信した BufferCore を受け、LeaseHandle::acquire(shm_name, slot_id, generation) を呼ぶ。
   * 共有メモリへの接続に成功し、generation が一致したら refcnt.fetch_add(1, acq_rel)。
...
public:
  ...
  // 取得：共有メモリへの接続 + generation一致確認 + refcnt++
...
* TypeAdapter（ROS→View） は、まず`LeaseHandle::acquire()`を試みる。
  * 成功：Lease が有効になった時点で BufferView の初期化を呼び、必要なら CUDA IPC open や VMM import を行う。
    * IPC ハンドルは BufferView/MemoryBackend 側のプロセス内キャッシュを参照し、`BufferView` の破棄時に
      `cudaIpcClose*` を呼ばない。
...
sequenceDiagram
  ...
  TA->>LHs: LeaseHandle::acquire(shm_name, slot_id, generation)  // SHM接続 + refcnt++
  alt acquire OK
