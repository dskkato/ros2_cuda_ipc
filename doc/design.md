...
BufferView は MemoryBackend 経由でハンドルを開き、開いた dev_ptr / ready_evt を
プロセス内キャッシュで共有する。これにより CopyConstructible という rclcpp の要件を
満たしつつ、フレームごとの open/close コストを排除する。TypeAdapter はこの初期化を呼び出す入口であり、
ハンドルの実体管理は BufferView/MemoryBackend 側に閉じ込める。
...
MemoryBackend は「BufferCore へのシリアライズ」「dev\_ptr 取得」「クリーンアップ」の 3 点をまとめる
小さなクラスで、BufferView は backend に応じた open 処理をキャッシュ層経由で呼び分けるだけで済む。
...
3. **BufferView と世代管理**
   * slot\_id + generation によりメモリ再初期化を検出し、必要に応じて FD の差し替えや VMM unmap を行う。
...
4. **VMM + FD 固有のリソース管理**
   ...
   * Publisher は slot を破棄または再初期化するときに、対応する socket を close/unlink し、export 元の
     CUDA VMM ハンドルを解放する。uuid は slot\_id + generation と対応付け、古い世代の socket/FD を
     新しい世代から参照しない。
   * Subscriber は import 済み FD と VMM mapping をプロセス内でキャッシュするが、generation 不一致や
     Publisher 再起動を検出した場合は該当 mapping を unmap し、必要なら再取得する。
   * 異常終了に備え、起動時には既存の `/tmp/cuda_memory_pool_<uuid>.sock` を検査し、自プロセスが所有する
     古い socket だけを削除する。別 Publisher の socket を誤って削除しないよう、uuid だけでなく
     shm\_name やプロセス識別子も管理情報に含める。
...
Adapter の責務は **Lease の取得、BufferView の初期化、ROS msg と View 間のメタデータコピー**。
...
* Subscriber: TypeAdapter 経由で BufferView を初期化し、BufferView/MemoryBackend が dev_ptr/event を取得する。
...
* Subscriber
  * TypeAdapter によって初期化済みの View を受け取る。
