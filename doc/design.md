...
BufferView 自体はハンドルを開かず、TypeAdapter が開いた dev_ptr / ready_evt と
LeaseHandle を保持する。TypeAdapter はプロセス内キャッシュを使って同じハンドルの open/import を
初回だけ実行する。これにより CopyConstructible という rclcpp の要件を満たしつつ、
フレームごとの open/close コストを排除する。
...
TypeAdapter が `BufferCore.backend` を見て CUDA IPC open または VMM import を呼び分ける。
...
4. **VMM + FD 固有のリソース管理**
   ...
   * Publisher は slot を破棄または再初期化するときに、対応する socket を close/unlink し、export 元の
     ...
     socket/FD を作り直さない。
   * Subscriber は import 済み FD と VMM mapping を TypeAdapter のプロセス内キャッシュで共有する。
...
Adapter の責務は **Lease の取得、IPC/VMM ハンドルの open/import とキャッシュ、BufferView の構築、
ROS msg と View 間のメタデータコピー**。
...
* Subscriber: TypeAdapter が BufferView を構築し、`BufferCore.backend` に応じて dev_ptr/event を取得する。
