...
現在の受信側 TypeAdapter は `ros2_cuda_ipc_core/include/ros2_cuda_ipc_core/type_adapters.hpp` と `backend/type_adapters.hpp` に実装されている。特に `backend/type_adapters.hpp` は CUDA Runtime API、CUDA Driver API、POSIX socket、libuuid、cache、VMM map まで含むため、TypeAdapter の変更影響範囲が広く、backend 追加、resource lifetime 修正、明示的なエラー処理、単体テストが難しい。
...
- `ros2_cuda_ipc_core/include/ros2_cuda_ipc_core/backend/type_adapters.hpp`
...
- `ros2_cuda_ipc_core/include/ros2_cuda_ipc_core/backend/backend_importer.hpp`
- `ros2_cuda_ipc_core/include/ros2_cuda_ipc_core/backend/cuda_ipc_importer.hpp`
- `ros2_cuda_ipc_core/include/ros2_cuda_ipc_core/backend/vmm_fd_importer.hpp`
...
- `ros2_cuda_ipc_core/src/backend/cuda_ipc_importer.cpp`
- `ros2_cuda_ipc_core/src/backend/vmm_fd_importer.cpp`

`type_adapters.hpp` は引き続き public header とするが、CUDA/POSIX/VMM の詳細 include を直接持たない状態を目標にする。`backend/type_adapters.hpp` は最終的に削除するか、互換用の最小 helper だけに縮小する。
...
class IpcHandleCache {
 public:
  ...
  std::optional<backend::ImportedBuffer> find(const IpcHandleKey& key);
...
1. `IpcHandleCache` と `ImportedBuffer` を新設し、key/hash/cache entry/mutex を `backend/type_adapters.hpp` から移す。
...
7. `backend/type_adapters.hpp` から CUDA/POSIX/VMM の詳細を削除する。不要になればファイル自体を削除する。
