...
/**
 * @brief Runtime state for a VMM-backed GPU block (address, FD server, etc.).
 *
...
BlockResources::BlockResources() = default;
BlockResources::~BlockResources() = default;
BlockResources::BlockResources(BlockResources&&) noexcept = default;
BlockResources& BlockResources::operator=(BlockResources&&) noexcept = default;

bool allocate_vmm_fd_memory(uint64_t frame_size_bytes, int device_index,
                            std::vector<BlockResources>& blocks) {
  if (!ensure_driver()) {
    return false;
  }
...
void destroy_vmm_fd_memory(std::vector<BlockResources>& blocks) noexcept {
  for (auto& block : blocks) {
