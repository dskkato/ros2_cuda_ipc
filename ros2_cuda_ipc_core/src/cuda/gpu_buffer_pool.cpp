...
std::unique_ptr<GpuBufferPool::MemoryBackend> make_backend(
  ...
  if (backend == MemoryBackendKind::VMM_FD) {
...
GpuBufferPool::GpuBufferPool(std::size_t slot_count, MemoryBackendKind backend,
...
bool GpuBufferPool::initialise(uint64_t byte_size, int device_index) {
  ...
  if (set_device_error != cudaSuccess) {
    RCLCPP_ERROR(logger_, "cudaSetDevice failed: %s",
                 cuda_error_to_string(set_device_error).c_str());
    return false;
  }
    ...
    if (slot.event != nullptr) {
      ...
      if (error != cudaSuccess) {
        RCLCPP_ERROR(logger_, "cudaEventDestroy failed for slot %u: %s",
                     slot.index, cuda_error_to_string(error).c_str());
      }
      slot.event = nullptr;
    }
  }
